...
import { drawCompound } from "./compoundScore.js";

async function generateCtp() {
    const generate_ctp = state.wasm.cwrap("generate_ctp", null, []);
...
function readCtpArray() {
    const ptr = state.wasm._get_ctp();

    const size = state.actualLength;
    const pitches = [];

    // int32 view into WASM memory
    const mem = state.wasm.HEAP32 || state.wasm.asm.HEAP32;
...
<mei xmlns="http://www.music-encoding.org/ns/mei" meiversion="5.1">
  ...
  <music>
    ...
    const cantusOnTop = Pitch.highest(state.cantus).stepsTo(Pitch.highest(state.ctp)) < 0;
