...
function playFrequencies() {
    let frequencies = state.cantus.map((p) => audio.freq.toHz(p));
    const duration = 0.5;
    ...
    noteObjects.forEach((note, i) => {
        audio.activeLitNotes.push(setTimeout(
    ...
    frequencies.forEach((freq, i) => {
        ...
        osc.onended = () => {
            audio.activeOscillators = audio.activeOscillators.filter((o) => o !== osc);
