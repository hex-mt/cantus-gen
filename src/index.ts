...
async function generateCantus(tonic: string, mode: number, length: number): Promise<Pitch[]> {
    // @ts-ignore
...
function playFrequencies(frequencies) {
    ...
    frequencies.forEach((freq, i) => {
        ...
        gain.gain.linearRampToValueAtTime(0.3, audioCtx.currentTime + i * duration + 0.01);
        gain.gain.linearRampToValueAtTime(0.06, audioCtx.currentTime + (i + 1) * duration);
        osc.start(time);
